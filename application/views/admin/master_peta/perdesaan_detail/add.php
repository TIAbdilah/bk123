    ...
    <div class="col-lg-12">
        <section class="panel">
            <header class="panel-heading">
                <?php echo $title_page . ' Detil Dawasan Kumuh (' . $perdesaan->nm_kawasan . ' | ' . ucwords($kategori) . ')' ?>
            </header>
            <div class="panel-body">
                <form class="form-horizontal" role="form" action="<?php echo site_url('master_peta/perdesaan_detail/process1/add/') ?>" enctype="multipart/form-data" method="POST">
                    <input type="hidden" name="inpKategori" value="<?php echo $kategori ?>" />
                    <input type="hidden" name="inpIdKawKumuh" value="<?php echo $id_perdesaan ?>" />
                                ...
                                if (substr($dt_ind->nomor, 0, 2) == '10') {
                                    ...
                                    <tr>
                                        ...
                                        if (substr($dt_ind->nomor, 0, 2) != $id_ind) {
                                            echo '<td width="3%"><a title="' . $dt_ind->kriteria . '" href="#">' . substr($dt_ind->nomor, 0, 2) . '</a></td>';
                                        } else {
                                            echo '<td width="3%">&nbsp;</td>';
                                        }
                                        ?>                
                                        <td width="3%"><?php echo substr($dt_ind->nomor, 2, 1) ?></td>
                                        <td width="20%"><?php echo $dt_ind->kriteria ?></td>
                                        <td width="24%">
                                            <?php
                                            echo generate_radio($dt_ind->field_name);
