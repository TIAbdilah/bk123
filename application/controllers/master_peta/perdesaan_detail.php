...
class Perdesaan_detail extends MY_Controller {
    ...
    public function add($kategori = null, $id_perdesaan = null) {
        $data['title_page'] = 'ADD';
        $data['kategori'] = $kategori;
        $data['id_perdesaan'] = $id_perdesaan;
        $data['perdesaan'] = $this->perdesaan_model->select_by_field(array('id_perdesaan' => $id_perdesaan))->row();
        $data['data_indikator'] = $this->indikator_model->select_by_field(array('modul'=>'perdesaan'))->result();
        $data['page_content'] = 'admin/master_peta/perdesaan_detail/add';
        $data['text'] = $this->text;
    ...
    public function edit($kategori, $id = null) {
        ...
        $data['kategori'] = $kategori;
        $data['data_indikator'] = $this->indikator_model->select_by_field(array('modul'=>'perdesaan'))->result();
        $data['data'] = $this->perdesaan_detail_model->select_by_field(array('id_perdesaan_detail' => $id))->row_array();
        $data['id_perdesaan'] = $data['data']['id_perdesaan'];
        $data['perdesaan'] = $this->perdesaan_model->select_by_field(array('id_perdesaan' => $data['id_perdesaan']))->row();
        $data['page_content'] = 'admin/master_peta/perdesaan_detail/edit';
        $data['text'] = $this->text;
        $this->load->view('admin/index', $data);
    }
    ...
    public function process($action, $id = null) {
        ...
        $data['id_perdesaan'] = $this->input->post('inpIdKawKumuh');
        ...
        $kmz_file = $this->upload_image(array('indikator_name' => 'kml_' . substr($data['kategori'], 0, 4), 'input' => 'inpKmlFile', 'id_perdesaan' => $data['id_perdesaan'], 'tipe_file' => 'kmz'));
        if ($kmz_file != '') {
            $data['kmz_file'] = $kmz_file;
        }

        $peta_file = $this->upload_image(array('indikator_name' => 'peta_' . substr($data['kategori'], 0, 4), 'input' => 'inpPeta', 'id_perdesaan' => $data['id_perdesaan'], 'tipe_file' => 'peta'));
        if ($peta_file != '') {
            $data['peta_file'] = $peta_file;
        }

        $sk_file = $this->upload_image(array('indikator_name' => 'sk_' . substr($data['kategori'], 0, 4), 'input' => 'inpSk', 'id_perdesaan' => $data['id_perdesaan'], 'tipe_file' => 'sk'));
        if ($sk_file != '') {
        ...
        foreach ($data_indikator as $dt_ind) {
            ...
            if (substr($dt_ind->id, 0, 1) < 8) {
                ...
                $foto_file = $this->upload_image(array('indikator_name' => $dt_ind->field_name . '_' . substr($data['kategori'], 0, 4).'_ft', 'input' => 'inp_' . $dt_ind->field_name . '_ft', 'id_perdesaan' => $data['id_perdesaan'], 'tipe_file' => 'image'));
                if ($foto_file != '') {
                    $data[$dt_ind->field_name . '_foto'] = $foto_file;
                }
                 $foto_file2 = $this->upload_image(array('indikator_name' => $dt_ind->field_name . '_' . substr($data['kategori'], 0, 4).'_ft2', 'input' => 'inp_' . $dt_ind->field_name . '_ft2', 'id_perdesaan' => $data['id_perdesaan'], 'tipe_file' => 'image'));
                if ($foto_file2 != '') {
                    $data[$dt_ind->field_name . '_foto'] = $foto_file2;
                }
                 $foto_file3 = $this->upload_image(array('indikator_name' => $dt_ind->field_name . '_' . substr($data['kategori'], 0, 4).'_ft3', 'input' => 'inp_' . $dt_ind->field_name . '_ft3', 'id_perdesaan' => $data['id_perdesaan'], 'tipe_file' => 'image'));
                if ($foto_file3 != '') {
        ...
        for ($i = 1; $i < 8; $i++) {
            $point_line = $this->upload_image(array('indikator_name' => 'point_line_' . substr($data['kategori'], 0, 4) . '_' . $i, 'input' => 'inp_point_line_' . $i, 'id_perdesaan' => $data['id_perdesaan'], 'tipe_file' => 'kmz'));
            if ($point_line != '') {
                $data['point_line_' . $i] = $point_line;
            }
            $vid = $this->upload_image(array('indikator_name' => 'vid_kawasan_' . substr($data['kategori'], 0, 4) . '_' . $i, 'input' => 'inpVideo_' . $i, 'id_perdesaan' => $data['id_perdesaan'], 'tipe_file' => 'video'));
            if ($vid != '') {
    ...
    public function delete($id = null) {
        $this->perdesaan_detail_model->delete(array('id_modul' => $id));
    ...
    public function upload_image($param) {
        $config = array(
            'overwrite' => TRUE,
            'file_name' => $param['indikator_name'] . '_' . $param['id_perdesaan']
        );
        //file size
        ...
        if (!empty($_FILES[$param['input']]['name']) 
            ...
            if ($this->upload->do_upload($param['input'])) {
                ...
                redirect('master_peta/perdesaan/view/' . $param['id_perdesaan']);
            }
        } else {
            return '';
        }
